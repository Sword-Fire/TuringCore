 ...
 */
@AutoRegister
object PlayerDataServiceImpl : Service(), PlayerDataService {

    /**
     * 注册一个玩家数据类。
     * @param T 玩家数据类型
     */
    override fun <T : PlayerData> register(clazz: KClass<T>) {
    ...
    override fun <T : PlayerData> getPlayerData(uuid: UUID, clazz: KClass<out PlayerData>): T {
        ...
        @Suppress("UNCHECKED_CAST")
        return data as? T ?: error("While getting data $identifier of player $uuid, failed to convert PlayerData to ${clazz.simpleName}")
    }

    override fun withOfflinePlayerData(uuid: UUID, action: OfflinePlayerDataContext.() -> Unit): Boolean {
    ...
    override fun withOfflinePlayerData(username: String, action: OfflinePlayerDataContext.() -> Unit): Boolean {
